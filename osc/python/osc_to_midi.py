# osc_to_midi.py: Advertises its OSC UDP service via mDNS so other clients can discover it.
...
latest_acc_y = None
latest_opt_value = 2  # Start with opt=1
...
def gyr_to_midi(x_deg, y_deg, z_deg):
    ...
    global latest_acc_y
    # Map pitch (y_deg, -360 to +360) to note in scale
    pitch_norm = (y_deg + 360) / 720  # 0 to 1
    scale_degree = int(pitch_norm * NOTES_PER_OCTAVE) % NOTES_PER_OCTAVE
    # Map roll (x_deg, -360 to +360) to octave
    roll_norm = (x_deg + 360) / 720  # 0 to 1
    ...
    if latest_acc_y is not None:
        velocity = int(max(0, min(127, ((latest_acc_y + 20) / 40) * 127)))
    else:
        velocity = 112
...
def gyr_to_cc(x_deg, y_deg, z_deg, mode='all'):
    global x_deg_accum, y_deg_accum, z_deg_accum
    ...
    cc_x = map_cc(x_deg_accum)
    cc_y = map_cc(y_deg_accum)
    cc_z = map_cc(z_deg_accum)
...
def osc_server_thread():
    ...
    dispatcher.map("/left/gyr", handle_gyr)
    dispatcher.map("/left/acc", handle_acc)
...
def main():
    # Advertise OSC service via mDNS so others on network can discover it
    instance_name = "parangole_to_midi"
    ...
    print(f"mDNS OSC service registered: {service_name} at {local_ip}:8000")
